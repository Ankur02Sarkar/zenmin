...
var pageCounter = {
    init: () => {
        ...
        pageCounter.input =
            pageCounter.container.getElementsByTagName("input")[0];
...
var progressBar = {
    ...
    incrementProgress: (progress) => {
        ...
        progressBar.element.style.transform =
            "translateX(-" + (100 - width) + "%)";
    },
    init: () => {
        setTimeout(() => {
            if (!pdf) {
                ...
                var loadingFakeInterval = setInterval(() => {
...
function showViewerUI() {
    document
        .querySelectorAll(".viewer-ui")
        .forEach((el) => el.classList.remove("hidden"));
    pageCounter.update();
}

const hideViewerUI = debounce(() => {
    ...
    if (!pageViews[0]) {
        ...
        gutterWidth =
            Math.round(
                Math.max(
                    64,
                    (window.innerWidth - pageViews[0].viewport.width) / 2,
                ),
            ) - 2;
    }
...
function setupPageDom(pageView) {
    ...
    if (pageView.annotationLayerFactory) {
        var annotationLayer =
            pageView.annotationLayerFactory.createAnnotationLayerBuilder(
                div,
                pdfPage,
                null,
                null,
...
const updateCachedPages = throttle(() => {
    if (currentPage == null) {
        return;
    }
    ...
    for (var i = 0; i < pageViews.length; i++) {
        ((i) => {
            ...
            if (
                Math.abs(i - currentPage) < pageBuffer &&
                !pageViews[i].canvas
            ) {
                redrawPageCanvas(i);
            }
...
function setUpPageAnnotationLayer(pageView) {
    if (pageView.annotationLayer) {
        pageView.annotationLayer.linkService.goToDestination = async (dest) => {
                    ...
                    setTimeout(
                        () => {
                            if (
                                ...
                                (currentPage &&
                                    Math.abs(currentPage - pageNumber) <
                                        pageBuffer)
                            ) {
                                ...
                                requestIdleCallback(
                                    () => {
                                        pdfPageView.pdfPage
                                            ...
                                            .then((text) => {
                                                pdfPageView.textLayer.setTextContent(
                                                    text,
                                                );
                                                pdfPageView.textLayer.render(0);
                                                pdfPageView.annotationLayer.render(
...
window.addEventListener("resize", () => {
    ...
    document.querySelectorAll(".viewer-ui").forEach((el) => {
...
function redrawPageCanvas(i, cb) {
    var canvasWrapperNode =
        pageViews[i].div.getElementsByClassName("canvasWrapper")[0];
    ...
    pageViews[i]
        .draw()
        .then(() => {
            setUpPageAnnotationLayer(pageViews[i]);
        })
        .then(cb);
}
    ...
    if (pageCount > 100) {
        ...
        for (var i = 0; i < pageViews.length; i++) {
            ((i) => {
                ...
                var needsScaleChange =
                    pageViews[i].scale < minimumAcceptableScale;
                ...
                if (needsScaleChange || !pageViews[i].canvas) {
                    ...
                    redrawPageCanvas(i, () => {
                        if (needsScaleChange) {
                            pageViews[i].cssTransform({
                                target: pageViews[i].canvas,
                            });
                        }
