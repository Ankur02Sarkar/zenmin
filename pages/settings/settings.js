document.title = l("settingsPreferencesHeading") + " | ZenMin";
...
function updateBlockingLevelUI(level) {
    ...
    if (
        document.querySelector(
            "#tracking-level-container .setting-option.selected",
        )
    ) {
        document
...
settings.get("filtering", (value) => {
    ...
    if (value && value.exceptionDomains && value.exceptionDomains.length > 0) {
        blockingExceptionsInput.value =
            value.exceptionDomains.join(", ") + ", ";
            ...
            checkbox.addEventListener("change", (e) => {
                settings.get("filtering", (value) => {
                    ...
                    if (e.target.checked) {
                        ...
                        var idx = value.contentTypes.indexOf(
...
/* dark mode setting - light/dark/system toggle */
var toggleLightMode = document.getElementById("toggle-light-mode");
var toggleDarkMode = document.getElementById("toggle-dark-mode");
var toggleSystemMode = document.getElementById("toggle-system-mode");

function updateDarkModeToggle(value) {
    toggleLightMode.classList.remove("active");
    toggleDarkMode.classList.remove("active");
    toggleSystemMode.classList.remove("active");

    if (value === 1 || value === true) {
        toggleDarkMode.classList.add("active");
    } else if (value === 2 || value === undefined || value === false) {
        toggleSystemMode.classList.add("active");
    } else {
        // -1 or 0 or anything else => light
...
toggleSystemMode.addEventListener("click", function () {
    settings.set("darkMode", 2);
    updateDarkModeToggle(2);
});

/* glass theme setting */
var themeOptions = document.querySelectorAll("#theme-selector .theme-option");

function updateThemeSelection(value) {
    themeOptions.forEach(function (option) {
        option.classList.remove("selected");
        if (option.getAttribute("data-theme") === value) {
...
settings.get("userscriptsEnabled", (value) => {
    if (value === true) {
...
settings.get("useSeparateTitlebar", (value) => {
    if (value === true) {
...
settings.get("updateNotificationsEnabled", (value) => {
    if (value === false) {
...
settings.get("collectUsageStats", (value) => {
    if (value === false) {
...
var homepageInput = document.getElementById("homepage-url");

settings.get("homepage", (value) => {
    if (value) {
        homepageInput.value = value;
    }
});

homepageInput.addEventListener("input", function (e) {
    settings.set("homepage", this.value);
});

/* Default zoom setting */

var defaultZoomInput = document.getElementById("default-zoom");

settings.get("defaultZoom", (value) => {
    if (value) {
        defaultZoomInput.value = value;
    } else {
        defaultZoomInput.value = 100;
    }
});

defaultZoomInput.addEventListener("input", function (e) {
    var value = parseInt(this.value);
    if (value >= 25 && value <= 500) {
        settings.set("defaultZoom", value);
    }
});

/* Clear browsing data */

var clearBrowsingDataButton = document.getElementById("clear-browsing-data");

clearBrowsingDataButton.addEventListener("click", () => {
    if (confirm(l("settingsClearBrowsingDataConfirm"))) {
        ipc.invoke("clearStorageData").then(() => {
            alert(l("settingsClearBrowsingDataSuccess"));
        });
    }
});
