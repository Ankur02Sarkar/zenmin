...
function isPermissionGrantedForOrigin(
    ...
    for (var i = 0; i < grantedPermissions.length; i++) {
        if (requestOrigin === grantedPermissions[i].origin) {
            ...
            if (
                requestPermission === "pointerLock" &&
                grantedPermissions[i].permission === "pointerLock"
            ) {
                return true;
            }
...
function hasPendingRequestForOrigin(requestOrigin, permission, details) {
    for (var i = 0; i < pendingPermissions.length; i++) {
        if (
            requestOrigin === pendingPermissions[i].origin &&
...
ipc.on("permissionGranted", (e, permissionId) => {
    for (var i = 0; i < pendingPermissions.length; i++) {
        if (
            permissionId &&
            pendingPermissions[i].permissionId === permissionId
        ) {
