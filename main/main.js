...
if (process.argv.some((arg) => arg === "-v" || arg === "--version")) {
    console.log("ZenMin: " + app.getVersion());
...
const isDebuggingEnabled = process.argv.some(
    (arg) => arg === "--debug-browser",
);
...
const browserPage = "zenmin://app/index.html";
...
function sendIPCToWindow(window, action, data) {
    if (window && window.isDestroyed()) {
        console.warn(
            "ignoring message " + action + " sent to destroyed window",
        );
        return;
    }
    ...
    if (window && getWindowWebContents(window).isLoadingMainFrame()) {
        ...
        setTimeout(() => {
            if (getWindowWebContents(window).isLoadingMainFrame()) {
                getWindowWebContents(window).once("did-finish-load", () => {
...
function createWindow(customArgs = {}) {
    ...
    // (since the screen ZenMin was previously open on could have been removed)
...
function createWindowWithBounds(bounds, customArgs) {
    const newWin = new BaseWindow({
        ...
        minHeight: 350,
        titleBarStyle: settings.get("useSeparateTitlebar")
            ? "default"
            : "hidden",
    ...
    const mainView = new WebContentsView({
        webPreferences: {
            ...
            additionalArguments: [
                ...
                ...(customArgs.initialTask
                    ? ["--initial-task=" + customArgs.initialTask]
                    : []),
            ],
        },
    });
    ...
    mainView.webContents.ipc.on("set-window-title", (e, title) => {
        newWin.title = title;
    });
  ...
  */
    ...
    mainView.webContents.on("before-input-event", (e, input) => {
...
app.on("ready", () => {
  ...
  but if that's happening, we shouldn't display anything */
    ...
    getWindowWebContents(newWin).on("did-finish-load", () => {
        // if a URL was passed as a command line argument (probably because ZenMin is set as the default browser on Linux), open it.
...
ipc.on("tab-state-change", (e, events) => {
    ...
    if (!sourceWindowId) {
        console.warn(
            "warning: received tab state update from window after destruction, ignoring",
        );
        return;
    }
...
ipc.on("request-tab-state", (e) => {
    const otherWindow = windows
        .getAll()
        .find((w) => getWindowWebContents(w).id !== e.sender.id);
    if (!otherWindow) {
        throw new Error(
            "secondary window doesn't exist as source for tab state",
        );
    }
...
ipc.handle("get-downloads-directory", () => {
...
const translatePage = "zenmin://app/pages/translateService/index.html";
const translatePreload =
    __dirname + "/pages/translateService/translateServicePreload.js";

app.on("ready", () => {
    ipc.on("page-translation-session-create", (e) => {
        ...
        translateWindow.webContents.once("did-finish-load", () => {
            translateWindow.webContents.postMessage(
                "page-translation-session-create",
                null,
                e.ports,
            );
        });
