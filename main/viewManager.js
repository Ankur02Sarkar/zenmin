...
function createView(existingViewId, id, webPreferences, boundsString, events) {
    ...
    events.forEach((event) => {
        view.webContents.on(event, function (e) {
            ...
            const eventTarget =
                getWindowFromViewContents(view.webContents) ||
                windows.getCurrent();
    ...
    */
        ...
        return {
            ...
            createWindow: (options) => {
                const view = new WebContentsView({
                    webPreferences: getDefaultViewWebPreferences(),
    ...
    view.webContents.on("ipc-message", (e, channel, data) => {
        ...
        const eventTarget =
            getWindowFromViewContents(view.webContents) || windows.getCurrent();
  ...
  */
    view.webContents.on("did-start-navigation", (event) => {
        if (event.isMainFrame && !event.isSameDocument) {
            ...
            const shouldHaveJS =
                !settings.get("filtering")?.contentTypes?.includes("script") ||
                event.url.startsWith("zenmin://");
            if (hasJS !== shouldHaveJS) {
                setTimeout(() => {
                    view.webContents.stop();
                    const currentWindow = getWindowFromViewContents(
                        view.webContents,
                    );
                    destroyView(id);
...
function setView(id, senderContents) {
    ...
    if (windows.getState(win).selectedView !== viewMap[id]) {
        ...
        win.getContentView()
            .children.slice(1)
            .forEach((child) => win.getContentView().removeChildView(child));
...
function focusView(id) {
    ...
    if (
        ...
        getWindowWebContents(
            getWindowFromViewContents(viewMap[id]?.webContents),
        ).focus();
        return true;
    }
}
...
function getWindowFromViewContents(webContents) {
    const viewId = Object.keys(viewMap).find(
        (id) => viewMap[id].webContents === webContents,
    );
    return windows
        .getAll()
        .find((win) => windows.getState(win).selectedView === viewId);
}
...
function loadURLInView(id, url, win) {
    ...
    if (!viewStateMap[id].loadedInitialURL) {
        ...
        viewMap[id].webContents.once("dom-ready", () => {
...
ipc.on("callViewMethod", (e, data) => {
    ...
    if (result instanceof Promise) {
        result.then((result) => {
            if (data.callId) {
                e.sender.send("async-call-result", {
                    callId: data.callId,
                    error: null,
                    result,
                });
...
ipc.handle("getNavigationHistory", (e, id) => {
    if (!viewMap[id]?.webContents) {
        return null;
    }
    const entries = [];
    const activeIndex =
        viewMap[id].webContents.navigationHistory.getActiveIndex();
    const size = viewMap[id].webContents.navigationHistory.length();

    for (let i = 0; i < size; i++) {
        entries.push(
            viewMap[id].webContents.navigationHistory.getEntryAtIndex(i),
        );
    }
