...
function listenForDownloadHeaders(ses) {
    ses.webRequest.onHeadersReceived((details, callback) => {
        if (details.resourceType === "mainFrame" && details.responseHeaders) {
            ...
            if (details.webContents) {
                sourceWindow = windows.windowFromContents(
                    details.webContents,
                )?.win;
            }
            if (!sourceWindow) {
            ...
            if (
                ...
                typeHeader.filter((t) => t.includes("application/pdf")).length >
                    0 &&
                !attachment
            ) {
    ...
    */
        if (
            ...
            (details.webContents
                .getURL()
                .startsWith("zenmin://app/pages/pdfViewer") ||
                details.webContents
                    .getURL()
                    .startsWith("zenmin://app/reader/") ||
                details.webContents.getURL() === "zenmin://app/index.html")
        ) {
            ...
            callback({
                responseHeaders: {
                    ...filteredHeaders,
                    "Access-Control-Allow-Origin": "zenmin://app",
