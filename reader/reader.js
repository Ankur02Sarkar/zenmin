...
function extractAndShowNavigation(doc) {
    ...
    siteIcon.addEventListener("load", () => {
    ...
    try {
        ...
        items = items.filter(
            (item, idx) =>
                itemURLSet.indexOf(new URL(item.href).toString()) === idx,
        );
