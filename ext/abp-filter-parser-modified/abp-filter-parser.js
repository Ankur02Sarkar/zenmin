...
function parseFilter(input, parsedFilterData) {
    ...
    if (
        ...
        parsedFilterData.regex = new RegExp(
...
function indexOfFilter(input, filter, startingPos) {
    ...
    for (var f = 0; f < filterParts.length; f++) {
        ...
        if (
            ...
            if (
                separatorCharacters.indexOf(input[index + part.length]) === -1
            ) {
                return -1;
            }
        }
...
function matchOptions(filterOptions, input, contextParams, currentHost) {
    if (
        (((filterOptions && filterOptions.elementTypes) ||
            defaultElementTypes) &
    ...
    */
        ...
        if (
            ...
            (isSameOriginHost(contextParams.domain, currentHost) ||
                isSameOriginHost(currentHost, contextParams.domain))
        ) {
            return false;
        }
        ...
        if (
            ...
            filterOptions.skipDomains.some((skipDomain) =>
                isSameOriginHost(skipDomain, contextParams.domain),
            )
        ) {
            return false;
        }
        ...
        if (
            ...
            !filterOptions.domains.some((domain) =>
                isSameOriginHost(domain, contextParams.domain),
            )
        ) {
            return false;
        }
        ...
        for (var i = start, len = end; i < len; i++) {
           ...
           */
                    ...
                    if (wildcardToken.length < 4) {
                        var wildcardToken2 =
                            parsedFilterData.wildcardMatchParts[1]
                                .split("^")[0]
                                .substring(0, 10);
                        if (
                            wildcardToken2 &&
...
function matchesFilters(filters, input, contextParams) {
    ...
    var leftAnchoredMatches =
        filters.leftAnchored.getStartingSubstringsOf(input);
    if (leftAnchoredMatches.length !== 0) {
        var len = leftAnchoredMatches.length;
        for (i = 0; i < len; i++) {
            if (
                matchOptions(
                    leftAnchoredMatches[i],
                    input,
                    contextParams,
                    currentHost,
                )
    ...
    var rightAnchoredMatches =
        filters.rightAnchored.getEndingSubstringsOfReversed(input);
    if (rightAnchoredMatches.length !== 0) {
        var len = rightAnchoredMatches.length;
        for (i = 0; i < len; i++) {
            if (
                matchOptions(
                    rightAnchoredMatches[i],
                    input,
                    contextParams,
                    currentHost,
                )
    ...
    var nonAnchoredStringMatches =
        filters.nonAnchoredString.getSubstringsOf(input);
