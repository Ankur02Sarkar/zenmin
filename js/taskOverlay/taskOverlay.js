...
function deleteTabFromOverlay(item) {
    var itemIsFocused =
        item.classList.contains("fakefocus") || document.activeElement === item;
    ...
    if (!successorTab) {
        ...
        successorTab =
            allTabs[allTabs.indexOf(item) - 1] ||
            allTabs[allTabs.indexOf(item) + 1];
    }
...
var taskOverlay = {
    ...
    addTaskDragging: () => {
        const sortable = new Sortable(taskContainer, {
            ...
            onEnd: (e) => {
                ...
                var droppedTask = tasks.splice(
                    tasks.getIndex(droppedTaskId),
                    1,
                )[0];
    ...
    addTabDragging: (el) => {
        const sortable = new Sortable(el, {
            ...
            onEnd: (e) => {
                ...
                const sortedItems = Array.from(e.to.children).filter((item) =>
                    items.some((item2) => item2 === item),
                );
                ...
                sortedItems.forEach((item) => {
                    ...
                    if (oldTab.selected) {
                        ...
                        var mostRecentTab = previousTask.tabs
                            .get()
                    ...
                    var newIdx = Array.from(e.to.children).findIndex(
                        (t) => t === item,
                    );
    ...
    render: function () {
        ...
        tasks.forEach((task, index) => {
            ...
            taskOverlay.addTabDragging(
                el.querySelector(".task-tabs-container"),
            );
        });
    },
    ...
    hide: function () {
        if (this.isShown) {
            ...
            for (var i = 0; i < pendingDeleteTasks.length; i++) {
                browserUI.closeTask(
                    pendingDeleteTasks[i].getAttribute("data-task"),
                );
            }
    ...
    initializeSearch: () => {
        ...
        input.addEventListener("input", (e) => {
            ...
            tasks.forEach((task) => {
                var taskContainer = document.querySelector(
        ...
        input.addEventListener("keypress", (e) => {
            if (e.keyCode === 13) {
                var firstTab = taskOverlay.overlayElement.querySelector(
    ...
    initialize: function () {
        ...
        keyboardNavigationHelper.addToGroup(
            "taskOverlay",
            taskOverlay.overlayElement,
        );
        ...
        keybindings.defineShortcut(
            ...
            (e) => {
                var focusedTab =
                    document.querySelector(".task-tab-item.fakefocus") ||
