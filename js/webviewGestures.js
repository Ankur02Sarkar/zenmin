    ...
    zoomWebviewBy: (tabId, amt) => {
        webviews.callAsync(tabId, "zoomFactor", (err, oldFactor) => {
            webviews.callAsync(
                ...
                Math.min(
                    webviewMaxZoom,
                    Math.max(webviewMinZoom, oldFactor + amt),
                ),
            );
        });
    },
...
function onSwipeGestureLowVelocity() {
    ...
    webviews.callAsync(tabs.getSelected(), "getZoomFactor", (err, result) => {
        const minScrollDistance = 150 * result;

        if (
            leftMouseMove / rightMouseMove > 5 ||
            rightMouseMove / leftMouseMove > 5
        ) {
...
webviews.bindIPC("wheel-event", (tabId, e) => {
    e = JSON.parse(e);
    ...
    swipeGestureDistanceResetTimeout = setTimeout(
        resetDistanceCounters,
        swipeGestureDelay,
    );
    swipeGestureScrollResetTimeout = setTimeout(
        resetScrollCounters,
        swipeGestureScrollDelay,
    );
