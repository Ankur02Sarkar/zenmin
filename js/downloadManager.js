...
const downloadManager = {
    ...
    hide: () => {
        if (downloadManager.isShown) {
            ...
            for (const item in downloadManager.downloadItems) {
                if (
                    downloadManager.downloadItems[item].status !== "progressing"
                ) {
    ...
    updateItem: (downloadItem) => {
        ...
        if (downloadItem.status === "completed") {
            ...
            elements.openFolder.hidden = true;
            elements.infoBox.textContent = getFileSizeString(
            ...
            const adjustedProgress =
                0.025 +
                (downloadItem.size.received / downloadItem.size.total) * 0.975;
            elements.progress.style.transform =
