...
class OnePassword {
    ...
    // First it checks if the tool was installed for ZenMin specifically
    ...
    async _completeIntegrationPrompt(command) {
        try {
            ...
            if (
                ...
                try {
                    const retryProcess = new ProcessSpawner(
                        command,
                        ["whoami"],
                        {},
                        1000,
    ...
    async checkIfConfigured() {
        ...
        return (
            this.path != null &&
            (await this._checkVersion(this.path)) &&
            (await this._completeIntegrationPrompt(this.path))
        );
    }
    ...
    isUnlocked() {
        return (
            this.sessionKey !== null &&
            Date.now() - this.sessionKeyCreated < this.sessionKeyLifetime
        );
    }
    ...
    async loadSuggestions(command, domain) {
        try {
            ...
            const credentials = matches.filter((match) => {
                try {
                    var matchHost = new URL(
                        match.urls.find((url) => url.primary).href,
                    ).hostname;
            ...
            for (var i = 0; i < credentials.length; i++) {
                ...
                var usernameFields = credential.fields.filter(
                    (f) => f.label === "username",
                );
                var passwordFields = credential.fields.filter(
                    (f) => f.label === "password",
                );
            ...
            const { error, data } = ex;
            console.error(
                "Error accessing 1Password CLI. STDOUT: " +
                    data +
                    ". STDERR: " +
                    error,
                ex,
    ...
    async signInAndSave(path = this.path) {
        ...
        const logoutProcess = new ProcessSpawner(
            path,
            ["signout"],
            { OP_DEVICE: this.deviceID },
            5000,
        ...
        const key = await process.executeSyncInAsyncContext(
