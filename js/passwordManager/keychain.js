...
class Keychain {
    ...
    async getSuggestions(domain) {
        return ipcRenderer
            .invoke("credentialStoreGetCredentials")
            .then((results) =>
                results
    ...
    deleteCredential(domain, username) {
        ipcRenderer.invoke("credentialStoreDeletePassword", {
            domain,
            username,
        });
    }
    ...
    async importCredentials(fileContents) {
        try {
            ...
            const credentialsToImport = csvData.data
                .map((credential) => {
                    try {
                        const includesProtocol =
                            credential.url.match(/^https?:\/\//g);
                        const domainWithProtocol = includesProtocol
                            ? credential.url
            ...
            const credentialsWithoutDuplicates = currentCredentials.filter(
                (account) =>
                    !credentialsToImport.some(
                        (a) =>
            ...
            const mergedCredentials =
                credentialsWithoutDuplicates.concat(credentialsToImport);
