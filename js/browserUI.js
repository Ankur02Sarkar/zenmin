...
function closeTask(taskId) {
    ...
    if (taskId === previousCurrentTask) {
        ...
        if (tasks.getLength() === 0) {
            ...
            var recentTaskList = tasks.map((task) => ({
                id: task.id,
                lastActivity: tasks.getLastActivity(task.id),
            }));
...
function switchToTask(id) {
    ...
    if (taskData.tabs.count() > 0) {
        ...
        if (!selectedTab) {
            selectedTab = taskData.tabs
                .get()
                .sort((a, b) => b.lastActivity - a.lastActivity)[0].id;
        }
