...
function extractTokens(text, tokens) {
    ...
    while (nextToken?.index !== -1) {
        nextToken = tokens
            .map((token) => ({
                token,
                index: remainingText.toLowerCase().indexOf(token),
            }))
            .reduce((a, b) => {
        ...
        if (nextToken.index !== -1) {
            ...
            out.push({
                token: remainingText.substring(
                    nextToken.index,
                    nextToken.index + nextToken.token.length,
                ),
            });
...
function createItem(data) {
    ...
    if (data.secondaryText) {
        ...
        if (data.metadata) {
            data.metadata.forEach((str) => {
                ...
                secondaryText.insertBefore(
                    metadataElement,
                    secondaryText.firstChild,
                );
            });
        }
    }
    ...
    if (data.descriptionBlock) {
        ...
        if (data.highlightedTerms) {
            extractTokens(data.descriptionBlock, data.highlightedTerms).forEach(
                (span) => {
    ...
    if (data.textActionButtons) {
        ...
        data.textActionButtons.forEach((buttonEntry) => {
            var button = document.createElement("button");
