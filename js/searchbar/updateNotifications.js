...
function showUpdateNotification(text, input, inputFlags) {
    function displayUpdateNotification() {
        ...
        searchbarPlugins.addResult(
            "updateNotifications",
            {
                ...
                descriptionBlock:
                    update.releaseHeadline || "View release notes",
    ...
    if (update) {
        ...
        if (
            compareVersions(window.globalArgs["app-version"], update.version) <=
            0
        ) {
            return;
        }
...
function initialize() {
    searchbarPlugins.register("updateNotifications", {
        index: 11,
        trigger: (text) => !text && performance.now() > 5000,
