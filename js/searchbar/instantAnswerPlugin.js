...
var instantAnswers = {
    color_code: (searchText, answer) => {
        ...
        var rgb = answer
            .split(" ~ ")
            .filter((format) => format.startsWith("RGBA"));
    ...
    currency_in: (searchText, answer) => {
        ...
        if (typeof answer === "string") {
            ...
            for (var countryCode in answer) {
                currencyArr.push(
                    answer[countryCode] + " (" + countryCode + ")",
                );
            }
        ...
        .then((res) => {
            ...
            if (
                ...
                if (searchbarPlugins.getTopAnswer()) {
                    ...
                    if (!inputFlags.isDeletion) {
                        ...
                        const autocompletionType =
                            searchbarAutocomplete.autocompleteURL(input, url);

                        if (autocompletionType !== -1) {
                            suggestedSiteData.fakeFocus = true;
                        }
                    }
                    searchbarPlugins.setTopAnswer(
                        "instantAnswers",
                        suggestedSiteData,
                    );
                }
            ...
            if (entitiesWithLocations.indexOf(res.Entity) !== -1) {
                searchbarPlugins.addResult("instantAnswers", {
                    icon: "carbon:search",
                    title: res.Heading,
                    secondaryText: l("searchWith").replace(
                        "%s",
                        "OpenStreetMap",
                    ),
                    classList: ["ddg-answer"],
