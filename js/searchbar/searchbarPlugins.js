...
const searchbarPlugins = {
    ...
    addResult: (pluginName, data, options = {}) => {
        ...
        if (data.urlKey && !data.allowDuplicates) {
            ...
            for (var plugin in results) {
                for (var i = 0; i < results[plugin].length; i++) {
                    if (
                        results[plugin][i].urlKey === data.urlKey &&
    ...
    register: (name, object) => {
        ...
        searchbar.insertBefore(
            container,
            searchbar.childNodes[object.index + 2],
        );
