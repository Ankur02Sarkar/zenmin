...
async function showSearchbarPlaceResults(
    text,
    input,
    inputFlags,
    pluginName = "places",
    ...
    results.forEach((result, index) => {
        ...
        if (
            ...
            if (result.searchFragment.contextBefore) {
                fragment +=
                    encodeURIComponent(
                        result.searchFragment.contextBefore,
                    ).replace(/-/g, "%2d") + "-,";
            }
            fragment += encodeURIComponent(
                result.searchFragment.fragment,
            ).replace(/-/g, "%2d");
            if (result.searchFragment.contextAfter) {
                fragment +=
                    ",-" +
                    encodeURIComponent(
                        result.searchFragment.contextAfter,
                    ).replace(/-/g, "%2d");
            }
            url += fragment;
        }
        ...
        if (searchQuery) {
            ...
            data.title = urlParser.prettyURL(
        ...
        function onSwitchClick(task) {
            ...
            browserUI.switchToTab(
                task.tabs
                    ...
                    .find(
                        (tab) =>
                            urlParser.removeTextFragment(tab.url) ===
                            urlComparisonBase,
                    ).id,
            );
        }
        ...
        if (
            ...
            !didAutocompleteResult &&
            urlComparisonBase !== tabs.get(tabs.getSelected()).url &&
            url !== tabs.get(tabs.getSelected()).url
        ) {
            data.textActionButtons = [];

            const currentTaskMatch = matchingTasks.find(
                (task) => task.id === tasks.getSelected().id,
            );
            ...
            matchingTasks
                .filter((task) => task.id !== currentTaskMatch?.id)
                .forEach((task) => {
                    const taskName = task.name
                        ? '"' + task.name + '"'
                        : l("defaultTaskName").replace(
                              "%n",
                              tasks.getIndex(task.id) + 1,
                          );
                    data.textActionButtons.push({
                        icon: "carbon:arrow-up-right",
                        text:
                            task.id === tasks.getSelected().id
                                ? l("placesPluginSwitchToTab")
                                : l("placesPluginSwitchToTask").replace(
                                      "%t",
                                      taskName,
                                  ),
                        fn: (e) => {
