...
function searchCustomBangs(text) {
    return customBangs.filter((item) => item.phrase.indexOf(text) === 0);
}
...
var saveBangUseCounts = debounce(() => {
...
function showBangSearchResults(text, results, input, inputFlags, limit = 5) {
    ...
    const allScores = results
        ...
        .filter((score) => !!score);
    const maxScore =
        allScores.length === 0 ? 0 : Math.max.apply(this, allScores);
