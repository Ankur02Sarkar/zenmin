    ...
    showMenu: (data, extraData) => {
        ...
        if (link) {
            ...
            linkActions.push({
                label: l("saveLinkAs"),
                click: () => {
                    webviews.callAsync(tabs.getSelected(), "downloadURL", [
                        link,
                    ]);
                },
            });
        ...
        if (mediaURL && data.mediaType === "image") {
            ...
            imageActions.push({
                ...
                click: () => {
                    browserUI.addTab(
                        tabs.add({ url: mediaURL, private: true }),
                        {
                            enterEditMode: false,
                            openInBackground: openInBackground,
                        },
        ...
        if (selection) {
            var textActions = [
                {
                    label: l("searchWith").replace(
                        "%s",
                        searchEngine.getCurrent().name,
                    ),
                    click: () => {
                        var newTab = tabs.add({
                            url: searchEngine
        ...
        if (
            data.formControlType === "input-password" &&
            PasswordManagers.getActivePasswordManager()?.saveCredential
        ) {
            menuSections.push([
                {
        ...
        menuSections.push([
            {
                ...
                click: () => {
                    webviews.callAsync(tabs.getSelected(), "inspectElement", [
                        data.x || 0,
                        data.y || 0,
                    ]);
                },
        ...
        translateLangList[0].forEach((language) => {
            translateMenu.submenu.push({
                label: language.name,
                click: () => {
                    pageTranslations.translateInto(
                        tabs.getSelected(),
                        language.code,
                    );
                },
        ...
        translateMenu.submenu.push({
            ...
            click: () => {
                browserUI.addTab(
                    tabs.add({
                        url:
                            "https://github.com/minbrowser/min/issues/new?title=Translation%20feedback%20for%20" +
                            encodeURIComponent(
