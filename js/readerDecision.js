...
const readerDecision = {
    ...
    */
        ...
        try {
            ...
            if (readerDecision.info.URLStatus[url]) {
                ...
                if (readerDecision.info.URLStatus[url].isReaderable === true) {
                    ...
                    readerDecision.info.URLStatus[url].isReaderable === false
                ) {
                    // we know it won't be readerable (or reader mode might be broken for the page), never redirect to it
                    return -1;
                }
            } else if (
                readerDecision.info.domainStatus[urlObj.hostname] === true
            ) {
    ...
    setDomainStatus: (url, autoRedirect) => {
        readerDecision.info.domainStatus[new URL(url).hostname] = autoRedirect;
        saveData();
    },
    getDomainStatus: (url) =>
        readerDecision.info.domainStatus[new URL(url).hostname],
...
settings.listen("readerData", (data) => {
    loadData(data);
    if (cleanupData()) {
        saveData();
    }
