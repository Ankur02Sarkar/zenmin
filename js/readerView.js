...
var readerView = {
    readerURL: "zenmin://app/reader/index.html",
    getReaderURL: (url) => readerView.readerURL + "?url=" + url,
    isReader: (tabId) =>
        tabs.get(tabId).url.indexOf(readerView.readerURL) === 0,
    getButton: (tabId) => {
        // TODO better icon
    ...
    printArticle: (tabId) => {
        if (!readerView.isReader(tabId)) {
            throw new Error(
                "attempting to print in a tab that isn't a reader page",
            );
        }
