...
var urlParser = {
    ...
    */
    ...
    isURLMissingProtocol: (url) => !urlParser.protocolRegex.test(url),
    parse: (url) => {
        ...
        if (url.startsWith("zenmin:") && !url.startsWith("zenmin://app/")) {
            // convert shortened zenmin:// urls to full ones
            const urlChunks = url
                .split("?")[0]
                .replace(/zenmin:(\/\/)?/g, "")
                .split("/");
    ...
    basicURL: (url) =>
        removeWWW(urlParser.removeProtocol(removeTrailingSlash(url))),
    prettyURL: (url) => {
        try {
            var urlOBJ = new URL(url);
            return removeWWW(
                removeTrailingSlash(urlOBJ.hostname + urlOBJ.pathname),
            );
        } catch (e) {
    ...
    isInternalURL: (url) => url.startsWith("zenmin://"),
    getSourceURL: (url) => {
        ...
        if (urlParser.isInternalURL(url)) {
            var representedURL;
            try {
                representedURL = new URLSearchParams(new URL(url).search).get(
                    "url",
                );
            } catch (e) {}
            if (representedURL) {
                ...
                try {
                    ...
                    var urlObj = new URL(url);
                    if (pageName) {
                        return "zenmin://" + pageName[1] + urlObj.search;
                    }
                } catch (e) {}
            }
        }
    ...
    validateDomain: (domain) => {
        ...
        return (
            publicSuffixes.find((s) => cleanDomain.endsWith(s)) !== undefined
        );
    },
    isHTTPSUpgreadable: (url) => {
