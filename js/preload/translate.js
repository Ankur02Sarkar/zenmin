...
async function translate(destLang) {
    ...
    for (var x = 0; x < frames.length; x++) {
        try {
            nodes = nodes.concat(
                getNodes(frames[x].contentDocument, frames[x].contentWindow),
            );
        } catch (e) {}
    }
    ...
    function handleChunk() {
        ...
        var selectionParent;
        try {
            selectionParent = window
                .getSelection()
                .getRangeAt(0).commonAncestorContainer;
        } catch (e) {}

        var sortedNodes = nodesSet
            .map((item) => {
                item.score = 0;
        ...
        channel.port2.onmessage = (e) => {
            e.data.translatedText.forEach((text, i) => {
                var rootNodeIndex = nodesSet.findIndex(
                    (item) => item.node === nodesInQuery[i],
                );
        ...
        */
                if (query[i].length > 2 && text.length / query[i].length > 20) {
                    console.warn(
                        "skipping possibly invalid translation: ",
                        query[i],
                        " -> ",
                        text,
                    );
        ...
        */
                if (destLang === "en") {
                    ...
                    if (
                        originalFirstChar &&
                        originalFirstChar.toUpperCase() === originalFirstChar
                    ) {
