...
const getKeyIcon = () => {
    ...
    const keyIconPolicy = trustedTypes.createPolicy(
        "minAutofillTrustedKeyIcon",
        {
...
function getPasswordConfirmationField(primaryField) {
    const autocompleteMarkedFields = Array.from(
        document.querySelectorAll('[autocomplete="new-password"]'),
    );
    ...
    if (autocompleteMarkedFields.length > 0) {
        ...
        const bestConfirmInput = getBestInput(
            ["confirm", "retype"],
            [],
            ["password"],
        );
...
const passwordGenerationCharset =
    "bcdfghjklmnpqrstvwxyzBCDFGHJKLMNPQRSTVWXYZ0123456789-_!";
...
ipc.on("generate-password", (location) => {
    var input = document.activeElement.matches("input[type=password]")
        ? document.activeElement
        : Array.from(document.elementsFromPoint(location.x, location.y)).filter(
              (el) => el.matches("input[type=password]"),
          );
    ...
    if (input) {
        ...
        if (priorGeneratedPassword) {
            ...
            values.forEach((value) => {
                generatedPassword +=
                    passwordGenerationCharset[
                        Math.floor(
                            (value / 256) * passwordGenerationCharset.length,
                        )
                    ];
            });
