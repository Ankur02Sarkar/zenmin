...
class TaskList {
        ...
        this.events.forEach((listener) => {
            if (listener.name === name || listener.name === "*") {
                this.pendingCallbacks.push([
                    listener.fn,
                    (listener.name === "*" ? [name] : []).concat(data),
                ]);
                ...
                if (!this.pendingCallbackTimeout) {
                    this.pendingCallbackTimeout = setTimeout(() => {
                        this.pendingCallbacks.forEach((t) =>
                            t[0].apply(this, t[1]),
                        );
                        this.pendingCallbacks = [];
                        this.pendingCallbackTimeout = null;
    ...
    update(id, data, emit = true) {
        ...
        if (!task) {
            throw new ReferenceError(
                "Attempted to update a task that does not exist.",
            );
        }
    ...
    getStringifyableState() {
        return {
            tasks: this.tasks
                .map((task) =>
                    Object.assign({}, task, {
                        tabs: task.tabs.getStringifyableState(),
                    }),
                )
                .map((task) => {
                    ...
                    Object.keys(task)
                        .filter(
                            (key) =>
                                !TaskList.temporaryProperties.includes(key),
                        )
    ...
    getCopyableState() {
        return {
            tasks: this.tasks.map((task) =>
                Object.assign({}, task, { tabs: task.tabs.tabs }),
            ),
        };
    }
    ...
    destroy(id, emit = true) {
        ...
        if (emit) {
            ...
            this.get(id).tabs.forEach((tab) =>
                this.emit("tab-destroyed", tab.id, id),
            );
    ...
    isCollapsed(id) {
        ...
        return (
            ...
            (task.collapsed === undefined &&
                Date.now() - tasks.getLastActivity(task.id) >
                    7 * 24 * 60 * 60 * 1000)
        );
    }
