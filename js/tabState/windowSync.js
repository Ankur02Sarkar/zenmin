    ...
    initialize: () => {
        ...
        ipc.on("tab-state-change-receive", (e, data) => {
            ...
            events.forEach(function (event) {
                ...
                if (
                    (event[0] === "task-destroyed" &&
                        event[1] === priorSelectedTask) ||
                    (event[0] === "tab-destroyed" &&
                        event[2] === priorSelectedTask &&
                        tasks.getSelected().tabs.count() === 1)
                ) {
                    ipc.invoke("close");
                ...
                switch (event[0]) {
                    ...
                    case "tab-splice":
                        tasks
                            .get(event[1])
                            .tabs.spliceNoEmit(...event.slice(2));
                        break;
                ...
                if (
                    ...
                    var newTaskCandidates = tasks
                        .filter(
                            (task) =>
                                task.tabs.isEmpty() &&
                                !task.selectedInWindow &&
                                !task.name,
                        )
                        .sort((a, b) => {
                            return (
                                tasks.getLastActivity(b.id) -
                                tasks.getLastActivity(a.id)
                            );
                        });
