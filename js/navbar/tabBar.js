...
const tabBar = {
    ...
    getTab: (tabId) => tabBar.tabElementMap[tabId],
    getTabInput: (tabId) => tabBar.getTab(tabId).querySelector(".tab-input"),
    setActiveTab: (tabId) => {
    ...
    updateTab: (tabId, tabEl = tabBar.getTab(tabId)) => {
        ...
        const isNewTab =
            tabData.url === "" ||
            tabData.url === urlParser.parse("zenmin://newtab");
        if (isNewTab) {
        ...
        var insecureIcon = tabEl.getElementsByClassName(
            "icon-tab-not-secure",
        )[0];
    ...
    handleDividerPreference: (dividerPreference) => {
    ...
    initializeTabDragging: () => {
        ...
        tabBar.dragulaInstance.on("drop", (el, target, source, sibling) => {
...
tabBar.container.addEventListener("drop", (e) => {
    ...
    var path = data.files[0]
        ? "file://" + electron.webUtils.getPathForFile(data.files[0])
        : data.getData("text");
    if (!path) {
        return;
    }
