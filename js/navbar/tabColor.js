...
const tabColor = {
    ...
    updateFromThemeColor: (color, tabId) => {
        if (!color) {
            tabs.update(tabId, {
                themeColor: null,
            });
    ...
    updateFromImage: (favicons, tabId, callback) => {
            ...
            () => {
                colorExtractorImage.onload = (e) => {
                    const backgroundColor =
                        getColorFromImage(colorExtractorImage);
                    const backgroundColorAdjusted =
                        adjustColorForTheme(backgroundColor);
    ...
    updateColors: () => {
        ...
        if (window.isDarkMode) {
            return setColor(
                defaultColors.darkMode[0],
                defaultColors.darkMode[1],
            );
        }
