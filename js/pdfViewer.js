...
const PDFViewer = {
    url: {
        base: "zenmin://app/pages/pdfViewer/index.html",
        queryString: "?url=%l",
    },
    isPDFViewer: (tabId) => tabs.get(tabId).url.startsWith(PDFViewer.url.base),
    printPDF: (viewerTabId) => {
        if (!PDFViewer.isPDFViewer(viewerTabId)) {
            throw new Error(
                "attempting to print in a tab that isn't a PDF viewer",
            );
        }
    ...
    savePDF: (viewerTabId) => {
        if (!PDFViewer.isPDFViewer(viewerTabId)) {
            throw new Error(
                "attempting to save in a tab that isn't a PDF viewer",
            );
        }
    ...
    handlePDFOpenEvent: (event, data) => {
        if (!data.tabId) {
            var matchingTabs = tabs
                .get()
                .filter((t) => urlParser.getSourceURL(t.url) === data.url)
                .sort((a, b) => {
        ...
        var PDFurl =
            PDFViewer.url.base +
            PDFViewer.url.queryString.replace(
                "%l",
