...
function fullTextQuery(tokens) {
     ...
     */
        historyInMemoryCache.forEach((item) => {
            ...
            for (var i = 0; i < tokens.length; i++) {
                if (
                    !tokenMatches[i].includes(item.id) &&
                    !itext.includes(tokens[i])
                ) {
                    matched = false;
                    break;
                }
        ...
        .then((queryResults) => {
            ...
            for (let i = 0; i < dl; i++) {
                ...
                for (let n = 1; n < indexList.length; n++) {
                    ...
                    if (distance < 50) {
                        totalWordDistanceBoost +=
                            (50 - distance) ** 2 * 0.000075;
                    }
                    if (distance === 1) {
                ...
                for (let x = 0; x < sl; x++) {
                    ...
                    const bmIdf = Math.log(
                        (historyInMemoryCache.length - nqi + 0.5) /
                            (nqi + 0.5) +
                            1,
                    );
                    ...
                    const scorePart2 =
                        (tf * (k1 + 1)) /
                        (tf + k1 * (1 - b + b * (indexLen / 500))); // 500 is estimated average page length
                ...
                const snippetSearchWords = searchText
                    .split(/\s+/g)
                    .map((w) =>
                ...
                const snippetIndex = doc.extractedText
                    ...
                    : [];
                const mappedWords = snippetIndex.map((w) =>
                    stemmer(w.toLowerCase().replace(nonLetterRegex, "")),
                );
                ...
                const actualSnippetBounds = [
                    maxBegin +
                        mappedWords
                            .slice(maxBegin, maxEnd + 1)
                            .findIndex((token) =>
                                snippetSearchWords.includes(token),
                            ),
                    maxBegin +
                        mappedWords
                            .slice(maxBegin, maxEnd + 1)
                ...
                const snippet = snippetIndex
                    .slice(maxBegin, maxEnd + 5)
                    .join(" ");
                if (snippet) {
                    doc.searchFragment = {
                        contextBefore: snippetIndex[actualSnippetBounds[0] - 1],
                        fragment: snippetIndex
                            .slice(
                                actualSnippetBounds[0],
                                actualSnippetBounds[1] + 1,
                            )
                            .join(" "),
