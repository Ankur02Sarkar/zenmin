var tagIndex = {
  ...
  getPageTokens: function (page) {
    ...
    try {
      ...
      if (
        (page.url.startsWith("file://") || page.url.startsWith("zenmin://")) &&
    ...
    for (var term of tokens) {
      for (var tag in tagIndex.termTags[term]) {
        ...
        if (tagIndex.tagCounts[tag] >= 2) {
          ...
          scores[tag] +=
            Math.pow(docsWithTag / (tagIndex.termDocCounts[term] || 1), 2) *
  ...
  getSuggestedItemsForTags: function (tags) {
    var set = historyInMemoryCache
      ...
      .map((p) => {
        return {
          page: p,
          tags: tagIndex.getAllTagsRanked(p).filter((t) => t.value >= 1.1),
        };
      });
