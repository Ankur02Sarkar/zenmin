var tagIndex = {
    ...
    getPageTokens: (page) => {
        ...
        var tokens = tokenize(
            (/^(http|https|file):\/\//.test(page.title) ? "" : page.title) +
                " " +
                urlChunk,
        );
        ...
        for (var term of tokens) {
            for (var tag in tagIndex.termTags[term]) {
                ...
                if (tagIndex.tagCounts[tag] >= 2) {
                    ...
                    scores[tag] +=
                        (docsWithTag / (tagIndex.termDocCounts[term] || 1)) **
                            2 *
                        (0.85 + 0.1 * Math.sqrt(tagIndex.termDocCounts[term]));
