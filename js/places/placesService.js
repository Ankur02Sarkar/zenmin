...
function loadHistoryInMemory() {
    ...
    db.places
        ...
        .then(() => {
            ...
            historyInMemoryCache.sort(
                (a, b) => calculateHistoryScore(b) - calculateHistoryScore(a),
            );
...
function handleRequest(data, cb) {
    ...
    if (action === "getPlaceSuggestions") {
        function returnSuggestionResults() {
            ...
            results = results.sort((a, b) => b.hScore - a.hScore);
...
ipcRenderer.on("places-connect", (e) => {
    e.ports[0].addEventListener("message", (e2) => {
        const data = e2.data;
